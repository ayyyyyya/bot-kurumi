...
export default async function on({ cht, Exp, store, ev, is }) {
  ...
  const timeoutMaling = 12 * 60 * 60 * 1000
...
function msToTime(duration) {
  ...
  const _MARKET_SPREAD = { stock: 0.016, crypto: 0.030 }
...
ev.on(
  ...
  async () => {
    ...
    if (now - data.cooldown.copet < cd) {
      ...
      let sisa = `${dur.hour > 0 ? dur.hour + ' jam ' : ''}${dur.minutes > 0 ? dur.minutes + ' menit ' : ''}${dur.seconds > 0 ? dur.seconds + ' detik ' : ''}`
...
ev.on(
  ...
  async () => {
    ...
    if (now - (data.cooldown.ngaji || 0) < cd) {
      ...
      const sisa = `${dur.hour > 0 ? dur.hour + ' jam ' : ''}${dur.minutes > 0 ? dur.minutes + ' menit ' : ''}${dur.seconds > 0 ? dur.seconds + ' detik' : ''}`.trim()
...
async function __FARM_REGISTER_RPG({ ev }) {
  if (global.__FARM_RPG_V3__) return
  ...
  const _padL = (s, n) => String(s ?? '').padStart(n, ' ')
  const _code = (s) => '```' + '\n' + String(s || '') + '\n' + '```'
  const _msToTime = (ms) => {
  ...
  const _box = (title, lines) => {
    const top = `╭─「 ${title} 」─╮`
    const mid = `├────────────────────────┤`
    const bot = `╰────────────────────────╯`
    const body = Array.isArray(lines) ? lines : [String(lines || '')]
    return _code([top, ...body, bot].join('\n'))
  }
  ...
  const _SEASONS = [
    {
      ...
      pests: 0.92,
      market: 1.03,
      tag: { spice: 1.15, fruit: 1.08, grain: 0.96, leaf: 0.92, root: 1.00, bean: 1.05 }
    },
    {
      id: 'hujan',
      name: 'Musim Hujan',
      ...
      pests: 1.18,
      market: 0.98,
      tag: { grain: 1.12, leaf: 1.10, root: 1.03, fruit: 0.95, spice: 0.92, bean: 0.98 }
    },
    {
      id: 'panen',
      name: 'Musim Panen',
      ...
      pests: 1.00,
      market: 1.07,
      tag: { grain: 1.10, fruit: 1.07, root: 1.06, spice: 1.03, leaf: 1.03, bean: 1.04 }
    },
    {
      id: 'peralihan',
      name: 'Musim Peralihan',
      grow: 1.00,
      yield: 1.00,
      pests: 1.05,
      market: 1.00,
      tag: { grain: 1.00, fruit: 1.00, root: 1.00, spice: 1.00, leaf: 1.00, bean: 1.00 }
    }
  ]
  ...
  const _getWorld = () => {
    ...
    if (seasonInfo.id === 'kemarau') weather = pick(0.65, 0.25, 0.08, 0.02)
    else if (seasonInfo.id === 'hujan') weather = pick(0.15, 0.20, 0.60, 0.05)
    else if (seasonInfo.id === 'panen') weather = pick(0.55, 0.30, 0.12, 0.03)
    else weather = pick(0.45, 0.30, 0.20, 0.05)
  ...
  const _plotLine = (plot, i) => {
    if (!plot) return `│ #${_padL(i, 2)} : Kosong`
    const now = Date.now()
    ...
    return `│ #${_padL(i, 2)} : ${name}\n│      ${st} | Air:${w}/3 | Pupuk:${ftxt || '-'} | Hama:${pests}`
  }

  const _jobsClean = (p) => {
    const now = Date.now()
    p.farm.jobs = (p.farm.jobs || []).filter(j => j && (Number(j.endAt) || 0) > 0 && (Number(j.startAt) || 0) > 0 && (Number(j.qty) || 0) > 0 && (Number(j.endAt) || 0) > 0 && (j.done ? false : true) && (j.endAt - now > -7 * 24 * 60 * 60 * 1000))
  }
  ...
  const _sellLine = (k, q, basePrice, qty) => {
    const mult = q ? (_FQ[q]?.mult || 1) : 1
    const unit = Math.floor(basePrice * mult)
    const total = unit * qty
    return { unit, total, line: `│ • ${k} x${_fmt(qty)} = ${_fmt(total)}` }
  }
  ...
  ev.on(
    { cmd: ['farmplot', 'petak'], listmenu: ['farmplot'], tag: 'rpg', isGroup: true },
    async ({ cht }) => {
      const pfx = cht.prefix || '.'
      const self = _ensurePlayer(cht.sender)
      if (!self) return cht.reply(`❌ Kamu belum terdaftar di RPG.\nDaftar dulu: ${pfx}daftar`)
  ...
  ev.on(
    ...
    async ({ cht }) => {
      ...
      for (const k of keys) {
        ...
        if (_isSeed(k)) {
          const b = _seedBase(k)
          seeds.push(`│ • ${b} (${_metaName(b)}) x${_fmt(q)}`)
          continue
        }
  ...
  ev.on(
    ...
    async ({ cht, args }) => {
      ...
      const harvestOne = (i) => {
        ...
        const needSpace = qty
        if (_invCount(self) + needSpace > self.farm.storage.cap) return { ok: false, why: `Gudang penuh (butuh ${needSpace} slot)` }
        ...
        return { ok: true, base, name: c.name, qty, made, exp }
      }
  ...
  ev.on(
    ...
    async ({ cht, args }) => {
      ...
      if (!q) {
        ...
        const detail = []
        for (const t of take.take) {
          const o = _sellLine(t.id, t.q, basePrice, t.qty)
          gain += o.total
          detail.push(o.line)
        }
  ...
  ev.on(
    ...
    async ({ cht, args }) => {
      ...
      for (const k of Object.keys(r.need)) {
        ...
        const meta = _CROPS[k] || _PRODUCTS[k] || _MAT[k]
        if (!meta) return cht.reply(`❌ Bahan tidak valid: ${k}`)
      ...
      const durUse = batch
      fac.dur = Math.max(0, fac.dur - durUse)
  ...
  ev.on(
    ...
    async ({ cht, args }) => {
      ...
      for (const j of pick) {
        ...
        const needSpace = totalOut
        if (_invCount(self) + needSpace > self.farm.storage.cap) continue
  ...
  ev.on(
    ...
    async ({ cht, args }) => {
      ...
      const meta = _CROPS[ord.item] || _PRODUCTS[ord.item]
      if (!meta) return cht.reply(`❌ Data order rusak.`)
      ...
      let pay = 0
      for (const t of take.take) {
        const mult = t.q ? (_FQ[t.q]?.mult || 1) : 1
        const unit = Math.floor((Number(ord.payPer) || 0) * mult)
        pay += unit * t.qty
      }
