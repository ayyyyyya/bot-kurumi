...
export default async function on({ cht, Exp, store, ev, is }) {
  ...
  ev.on(
    ...
    async ({ media }) => {
      ...
      const message = {
        image: { url: 'https://files.catbox.moe/rdtk3c.jpg' }, 
        caption: `${tmp}`,
      ...
      return await Exp.sendMessage(cht.id, message, { quoted: cht });
    }
  );
  ...
  ev.on(
    {
      cmd: ['getchid', 'cekidch', 'getchannelid', 'getsaluranid', 'getidsaluran'],
      listmenu: ['getchid'],
      tag: 'tools',
      energy: 10,
