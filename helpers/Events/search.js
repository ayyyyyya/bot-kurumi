...
export default async function on({ cht, Exp, store, ev, is }) {
  ...
  ev.on(
    ...
    async ({ args }) => {
      ...
      let { data } = res || {}
      if (data.length < 1) return reply(args + 'tidak ditemukan...')
      let cards = []
      data = data.slice(0, 5)
      ...
      for (let i of data) {
        ...
        cards.push({
          ...
          body: { text: `foto ke-${cards.length + 1}` },
          footer: {
        ...
        id,
        {
          viewOnceMessage: {
            message: {
              interactiveMessage: {
                body: {
                  text: "✨Yeyyy " + botname + " berhasil dapat fotonya\nBerikut hasil dari " + args,
                },
                carouselMessage: {
                  cards,
                  messageVersion: 1,
  ...
  ev.on(
    {
      cmd: ['pinstik', 'pins', 'pinstick', 'pinstiker', 'pinsticker'],
      listmenu: ['pinsticker'],
      tag: 'search',
      energy: 5,
          ...
          if (data) {
            ...
            for (let i of data) {
              ...
              cards.push({
                ...
                body: { text: i.title },
                footer: { text: "Dapatkan kesempatan untuk mengakses semua fitur yang tersedia dengan membeli *premium*, silahkan clik tombol dibawah untuk neli premium (๑˃̵ᴗ˂̵)ﻭ" },
                nativeFlowMessage: {
                  buttons: [
                    {
                      name: "cta_url",
