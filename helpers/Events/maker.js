...
export default async function on({ cht, Exp, store, ev, is }) {
  ...
  ev.on(
    {
	  ...
	  tag: "maker",
	  energy: 15,
	  args: `*❗ Harap masukkan teks*`
	}, 
	async ({ args }) => {
        let buff = await func.getBuffer(`https://api.botcahx.eu.org/api/maker/brat?text=${encodeURIComponent(args)}&apikey=${cfg.bar}`) 
		let res = await exif["writeExifImg"](buff, {
          packname: '˖ ݁𖥔 ݁˖   𐙚   ˖ ݁𖥔 ݁˖\n',
	      author: cfg.sticker.author
		})
  ...
  ev.on(
    {
      cmd: ['brat2', 'bart2', 'bratgenerator2'],
      listmenu: ['brat'],
      tag: 'maker',
      energy: 15,
  ...
  ev.on(
    {
	  cmd: ['brat3', 'bart3', 'bratgenerator3'],
   	  listmenu: ['brat3'],
	  tag: "maker",
	  energy: 15,
	  args: `*❗ Harap masukkan teks*`
	...
	async () => {
        let buff = await func.getBuffer(`https://aqul-brat.hf.space/?text=${encodeURIComponent(cht.q)}`) 
		let res = await exif["writeExifImg"](buff, {
          packname: '˖ ݁𖥔 ݁˖   𐙚   ˖ ݁𖥔 ݁˖\n',
	      author: cfg.sticker.author
		})
...
ev.on(
  {
    ...
    tag: 'maker',
    energy: 15,
    args: `contoh: ${cht.msg} kalau 2 hsti ketemu di jalan bakal jadi apa?`,
  },
  async ({ args, cht }) => {
    let emoji = cht.cmd.includes("wa") ? 'whatsapp' : 'ios';

    let text = args.split(/--|-t\s|--time\s/)[0]?.trim();

    let timestamp =
      args.match(/--timestamp=(\S+)/)?.[1] ||
      args.match(/--time\s+(\S+)/)?.[1] ||
      args.match(/-t\s+(\S+)/)?.[1];
    ...
    Exp.sendMessage(
      cht.id,
      {
        ...
        caption: "✅ Berhasil membuat fake iqc"
      },
      { quoted: cht }
    );
  }
  ...
  ev.on(
     {
		...
		tag: "maker",
		energy: 20,
		args: `*❗Haral masukkan teks*`
	}, 
	async ({ args }) => {
        let buff = await func.getBuffer(`https://api.botcahx.eu.org/api/maker/brat-video?text=${encodeURIComponent(args)}&apikey=${cfg.bar}`)
