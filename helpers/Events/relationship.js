...
const { profileUser } = await (fol[2] + 'sharpAll.js').r()
...
export default async function on({ cht, Exp, store, ev, is }) {
  ...
  const energy = async (en) => {
    ...
    await cht.reply(`*${en} ᴇɴᴇʀɢʏ ᴛᴇʀᴘᴀᴋᴀɪ⚡*`);
  ...
  ev.on(
    ...
    async () => {
      ...
      try {
        ...
        url = 'https://c.termai.cc/i14/q5ybQ.jpg';
      }
      
      let resUrl = await profileUser(url)
      ...
      let txt = "`乂  P R O F I L E  乂`"
         txt += "\n```"
         txt += "\n• Nama      ′: " + (await func.getName(Sender))
         txt += `\n• Ultah     ′: ${user.ultah?.tanggal ? user.ultah.tanggal + "-" + (user.ultah.tahun || "") : "-"}`
         txt += `\n• Status    ′: ${user.pasangan?.status || "jomblo abadi"}`
         txt += "\n• Hubungan  ′: " + user.role
         txt += "\n• Obrolan   ′: " + user.chat
         txt += "\n• Energy    ′: " + user.energy
         txt += `\n• Premium   ′: ${user.premium.time >= Date.now() ? "✅" : "❌"}`
      if (user.premium.time >= Date.now()){
         let active = `${formatDur.days > 0 ? formatDur.days + 'd, ' : ''}${formatDur.hours > 0 ? formatDur.hours + 'h, ' : ''}${formatDur.minutes > 0 ? formatDur.minutes + 'm, ' : ''}${formatDur.seconds > 0 ? formatDur.seconds + 's' : ''}`
         txt += "\n  ⤷ Aktif selama : " + active
         txt += `\n  ⤷ Expired      : ${func.dateFormatter(user.premium.time, "Asia/Jakarta")}`
         txt += "\n```"
      } else {
         txt += "\n```"
      }
         txt += "\n`乂 C H A R G I N G 乂`"
         txt += "\n```"
         txt += `\n• Mode      ′: ${user.charging ? "🟢On" : "🔴Off"}`
         txt += "\n• Kecepatan ′: " + (parseFloat(user.chargeRate) + parseFloat(bonus.chargeRate)) + "/" + speed
         txt += "\n• Maksimum  ′: " + (parseFloat(user.maxCharge) + parseFloat(bonus.maxCharge))
         txt += "\n• Selesai   ′: " + func.dateFormatter(user.lastCharge, "Asia/Jakarta")
         txt += "\n```"
         txt += "\nbantu bot kecil ini untuk berkembang dan bertahan lama dengan donasi seikhlasnya :)"
      ...
      const menu = {
        ...
        contextInfo: {
          externalAdReply: {
            ...
            body: botname + ' artificial intelligence',
            thumbnail: resUrl,
            sourceUrl: cfg.ig,
  ...
  ev.on(
    ...
    async () => {
      ...
      let txt = " ▪︎ *[🔋] 𝐄𝐍𝐄𝐑𝐆𝐘*"
        txt += "\n*⚡ᴇɴᴇʀɢʏ:* " + user.energy
        txt += `\n\n*ᯓ ᴍᴏᴅᴇ:* ${user.charging ? "🟢Charging" : " ⚫Full"}`
        txt += "\n*ᯓ ᴋᴇᴄᴇᴘᴀᴛᴀɴ ᴄᴀꜱ:* " + rate + "/" + speed
        txt += "\n*ᯓ ᴍᴀᴋꜱɪᴍᴜᴍ ᴄᴀꜱ:* " + max
          if(user.charging){
            txt += "\n*ᯓ ᴘᴇʀᴋɪʀᴀᴀɴ:* " + estimate
            txt += "\n*ᯓ ꜱᴇʟᴇꜱᴀɪ:* " + finish
          } else {
            txt += "\n*ᯓ ꜱᴇʟᴇꜱᴀɪ ꜱᴇᴛᴇʟᴀʜ:* " + func.dateFormatter(user.lastCharge, "Asia/Jakarta")
          }
      Data.users[sender.split('@')[0]] = user;
      const mess = {
        text: txt,
        contextInfo: {
          externalAdReply: {
            ...
            sourceUrl: `${cfg.ig}`,
            mediaUrl: `http://ẉa.me/${owner[0]}/${user.charging ? '2733' : '2734'}`,
            renderLargerThumbnail: true,
            showAdAttribution: true,
            mediaType: 1,
          },
  ...
  ev.on(
    ...
    async () => {
      ...
      let txt = "`𝗯𝗲𝗿𝗵𝗮𝘀𝗶𝗹 𝗺𝗲𝗻𝗴𝗸𝗹𝗮𝗶𝗺 𝗽𝗿𝗲𝗺𝗶𝘂𝗺 𝗴𝗿𝗮𝘁𝗶𝘀` ⌛\n\n"
