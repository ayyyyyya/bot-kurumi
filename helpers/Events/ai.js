...
- CFG Scale: ${body.cfgScale}`.trim(),
  ...
  ev.on(
    {
      cmd: ['autoai', 'aichat', 'aiinteractive'],
      tag: 'ai',
      listmenu: ['autoai'],
    },
    async () => {
  ...
  ev.on(
    {
      cmd: ['faceswapreset', 'faceswapchange'],
      listmenu: ['faceswapreset ℗', 'faceswapchange ℗'],
      tag: 'ai',
      premium: true,
    },
    async () => {
  ...
  ev.on(
    {
      ...
      premium: true,
      energy: 30,
      args: "*❗ Harap setakan prompt nya dan prompt harus dalam bahasa inggris*",
      media: {
        type: ['image'],
      },
    },
