...
const ownerBarzz = owner[0] + from.sender
      ...
      number || id,
      {
        ...
        contextInfo: {
          externalAdReply: {
            ...
            mediaUrl: `http://ẉa.me/${owner[0]}/${!cust ? '89e838' : 'jeie337'}`,
            sourceUrl: `https://wa.me/${owner[0]}?text=Hello,+I+have+buy+Premium`,
...
`;
            if (vv == 'onlyjoingc') {
              if (!cfg.urlgc) {
                if (!t3) {
                  ...
                  cfg.urlgc ??= [];
                  cfg.urlgc = [...new Set([...cfg.urlgc, ...urls])];
                  let list = cfg.urlgc.map((a) => `- ${a}`).join('\n');

                  return cht.reply(
                    `Link grup berhasil ditambahkan!\n\`LIST INVITELINK\`\n${list}\n\nMode *Only Join GC* sekarang aktif. Pengguna non-premium harus bergabung ke salah satu grup resmi diatas sebelum dapat menggunakan bot.\n_Terima kasih telah menambahkan link grup._${iAdL}`
                  );
                }
              } else {
                let list = cfg.urlgc.map((a) => `- ${a}`).join('\n');
                return cht.reply(
                  `${func.tagReplacer(infos.owner.isModeOnSuccess, { mode })}!\n\n\`LIST INVITELINK\`\n${list}\n\nMode *Only Join GC* sekarang aktif. Pengguna non-premium harus bergabung ke grup resmi diatas sebelum dapat menggunakan bot${iAdL}`
                );
              }
            }
          } else {
  ...
  ev.on(
    ...
    async () => {
      ...
      let txt = `*Berhasil men${cht.cmd == "addenergy" ? "ambahkan":"gurangi"} ${num} ⚡Energy ke @${sender}*`
            txt += "\n\n*[🔋] Energy*"
            txt += "\n⚡Energy: " + user.energy
            txt += `\n\n*ᯓ Status:* ${user.charging ? "🟢Charging" : " ⚫Discharging"}`
            txt += "\n*ᯓ Charging Speed:* " + rate + " Energy/" + speed
            txt += "\n*ᯓ Max Charge*: " + max
      Exp.sendMessage(
        cht.id,
  ...
  ev.on(
    ...
    async ({ cht }) => {
      ...
      if (!is.owner) return cht.reply('Malasss 🧢');
      ...
      let opts = {
        addpremium: {
          ...
          msg: `*✅ Berhasil menambahkan durasi premium*
          
👤 User: @${Sender}
📅 Waktu ditambahkan:
▪︎ \`${formatDur.days}hari ${formatDur.hours}jam ${formatDur.minutes}menit ${formatDur.seconds}detik ${formatDur.milliseconds}ms\`\n\n─────────────────\n\n`,
        },
        addprem: {
          time: parseFloat(date) + parseFloat(premiumTime),
          msg: `*✅ Berhasil menambahkan durasi premium*
...
▪︎ \`${formatDur.days}hari ${formatDur.hours}jam ${formatDur.minutes}menit ${formatDur.seconds}detik ${formatDur.milliseconds}ms\`\n\n─────────────────\n\n`,
        ...
        kurangpremium: {
          time: parseFloat(date) - parseFloat(premiumTime),
          msg: `*✅ Berhasil mengurangi durasi premium*
...
▪︎ \`${formatDur.days}hari ${formatDur.hours}jam ${formatDur.minutes}menit ${formatDur.seconds}detik ${formatDur.milliseconds}ms\`\n\n─────────────────\n\n`,
        ...
        delpremium: {
          time: 0,
          msg: `*✅ Berhasil menghapus @${Sender} dari daftar user premium..*\n\n─────────────────\n\n`,
        },
        delprem: {
          time: 0,
          msg: `*✅ Berhasil menghapus @${Sender} dari daftar user premium...\n\n─────────────────\n\n`,
      ...
      let txt = opts[cht.cmd].msg
         txt += `💎 Premium: ${user.premium.time >= Date.now() ? "✅":"❌"}`
         if(user.premium.time >= Date.now()){
            ...
            txt += `\n⏳ \`Berakhir setelah:\`\n${formatTimeDur.days} hari, ${formatTimeDur.hours} jam\n${formatTimeDur.minutes} menit ${formatTimeDur.seconds} detik ${formatTimeDur.milliseconds} ms`
            txt += `\n\n📅 \`Expired:\`\n${func.dateFormatter(user.premium.time, "Asia/Jakarta")}`
            txt += txc
         } else {
           txt += `\n⏳ \`Expired setelah:\`\n-`
           txt += `\n\n🗓️ \`Expired:\`\n-`
...
ev.on({
  ...
  try {
    const progress = await cht.reply(loadingBar[0]);

    for (let i = 1; i < loadingBar.length; i++) {
      await sleep(250);
      await cht.edit(loadingBar[i], progress.key, true);
    }

    const sessions = fs.readdirSync(session).filter(file => file !== "creds.json");
    for (const file of sessions) {
      fs.unlinkSync(`${session}/${file}`);
      await sleep(130);
    }
    await cht.reply("✅ Berhasil membersihkan sesision...")

  } catch (err) {
    console.error("Error saat clear session:", err);
    await cht.reply("❌ Gagal membersihkan session.");
  }
});
  ...
  ev.on(
    ...
    async ({ urls }) => {
      ...
      if (cht.cmd == 'addlinkgc') {
        ...
        cfg.urlgc = cfg.urlgc.filter((a) => !cfg.urlgc.includes(a));
      }
      cht.reply(
  ...
  ev.on(
    ...
    async () => {
      if (cht.sender !== ownerBarzz) return
