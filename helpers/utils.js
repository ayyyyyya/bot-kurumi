...
export default async function utils({ Exp, cht, is, store }) {
  try {
    ...
    let isEdit = isProtocol && cht.message[type].type === 14; 
    
    if (isProtocol && !isDelete && !isEdit) return 'SKIP';
      ...
      cht.id === 'status@broadcast'
        ...
        : Data.setCmd[
          ...
          [
            ...
            { type: 'buttonsResponseMessage', msg: cht?.message?.[type]?.selectedButtonId },
            {
    ...
    is.newsletter = cht.id?.endsWith("@newsletter")
    is.baileys = /^(BAE5|BELL409|Laurine|B1E)/.test(cht.key.id);
    ...
    cht.warnGc = async ({ id, type, warn, kick, max }) => {
      ...
      if (groupDb.warn[jid][t].value > max) {
        ...
        await cht.reply(
          "‼️ `WARNING` ‼️\n\n" +
          `*Peringatan ke ${groupDb.warn[jid][t].value}*\n\n${warn}\n\n` +
